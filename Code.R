...
new_data <- rbind(colnames(new_data), new_data)
final_data <- new_data[, colnames(new_data) %in% exp_samples$Samples]
head(final_data) # Final dataset - # 17754 genes and 575 samples after QC
...
h_t <- hist(vals1, breaks = 20, col = "grey", labels = TRUE,
            ...
            xlim = c(0,8000))
xfit<- seq(min(vals1), max(vals1), length = 516)
...
legend(4000, 180, legend = c("Normal Curve", "Mean = 1623.751", 
       "Median = 1491.432"), col = c("red", "blue", "dark green"), 
       lwd = 3)
mean(vals1) # 1623.751
median(vals1) # 1491.432
...
h_s <- hist(vals1, breaks = 20, col = "grey", labels = TRUE,
            ...
            xlim = c(0,8000))
abline(v = 1091.8991, col = "blue", lwd = 3)
abline(v = 1490.2606, col = "black", lwd = 3)
abline(v = 2022.3598, col = "dark green", lwd = 3)
legend(3000, 200, legend = c("Normal Curve", "1st quartile 25%", 
                          "Median = 50%", "3rd quartile 75%"), 
       col = c("red", "blue", "black", "dark green"), 
       lwd = 3)
                          
q1 <- SMARCA2_tumor[SMARCA2_tumor$value<=1091.8991,]
q2 <- SMARCA2_tumor[SMARCA2_tumor$value>1091.8991 & SMARCA2_tumor$value<2022.3598,]
q3 <- SMARCA2_tumor[SMARCA2_tumor$value>=2022.3598,]
...
View(signif)
ones <- 0  # 6033 significant genes
zeros <- 0 # 11720 non-significant genes
for(i in signif){
  if(i == 1){
    ones = ones+1
  }else{
...
#-------------------------------------------------------
## Plotting top 15 significant genes (till p-value E-28)
#-------------------------------------------------------
# DIABLO
...
# C6orf129
compare_means(`C6orf129|154467` ~ BRM_level, data = signif_genes)
p <- ggboxplot(data = signif_genes, x = "BRM_level", 
               y = "C6orf129|154467", color = "BRM_level", 
               palette = "lancet", add = "jitter", 
               ylab = "C6orf129 gene expression", 
               main = "Boxplot of C6orf129")
p + stat_compare_means()
...
length(geneIDs) # 6033 significant genes
